...
int main(int argc, char *argv[]) {
    ...
    struct main_table* main_table = create_main_table(number_of_blocks);
    ...
    for (int i = 2; i < argc; i++) {
        if (strcmp(argv[i], "wc_files") == 0) {
            ...
            time_it(message, {
                wc_files(files, argv[i], number_of_files);
            });
        ...
        if (strcmp(argv[i], "read_file") == 0) {
            ...
            time_it(message, {
                struct block* block = read_file_to_block(filename);
        ...
        if (strcmp(argv[i], "remove_block") == 0) {
            ...
            time_it(message, {
                free_block(main_table -> blocks[index]);
