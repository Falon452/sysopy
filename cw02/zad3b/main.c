...
#define MAX_FILES_LOOKUP 1000000
...
int no_directory_files = -1;            //! nftw also counts given directory
int no_FIFO_files = 0;
...
static int file_stats(const char* filepath, const struct stat *_sb,
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 1) {
        ...
        nftw(dir_name, file_stats, MAX_FILES_LOOKUP, flags); // FTW-PHYS do not follow symbolic links
    } else
         nftw(argv[1], file_stats, MAX_FILES_LOOKUP, flags);  // FTW-PHYS do not follow symbolic links

    printf("----------------------------------------\n");
    printf("Regular files          count:   %d\n", no_regular_files);
    printf("Directory files in dir count:   %d\n", no_directory_files);
